...
const Sales = () => {
    ...
    useEffect(() => {
        ...
        getCustomerHistory(selectedCustomerId)
            .then((history) => {
                if (!active) return
                const nextVehicles = history?.vehicles ?? []
                setVehicles(nextVehicles)
                setSelectedVehicleId(nextVehicles[0]?.vehicleId ? String(nextVehicles[0].vehicleId) : '')
            })
            .catch(() => {
                if (!active) return
                setVehicles([])
                setSelectedVehicleId('')
                showToast('Failed to load customer vehicles.', 'error')
            })
            .finally(() => {
    ...
    const selectedVehicle = vehicles.find((vehicle) => vehicle.vehicleId === Number(selectedVehicleId))
    ...
    const handleSubmit = async () => {
        ...
        if (!selectedVehicleId) {
            showToast('Select a vehicle for this sale.', 'error')
            return
        }
                                    ...
                                    <div className="grid gap-4 md:grid-cols-2">
                                            ...
                                            >
                                                ...
                                                {customers.map((customer) => (
                                                    <option key={customer.userId} value={customer.userId}>
                                                        {customer.fullName} - {customer.vehicleNumber || customer.customerId}
                                                    </option>
                                                ))}
                                            </select>
                                        </label>
                                        ...
                                        <label className="space-y-2">
                                            ...
                                            <select
                                                ...
                                                disabled={!selectedCustomerId || vehicleLoading || vehicles.length === 0}
                                                className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-slate-400 disabled:bg-slate-50"
                                            >
                                                <option value="">
                                                    {!selectedCustomerId
